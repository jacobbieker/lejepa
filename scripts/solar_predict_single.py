...
def main():
    mlflow.start_run(run_name="single_prediction")
    batch_size = 256
    ...
    probe = nn.Sequential(nn.LayerNorm(512), nn.Linear(512, 1)).to(
    ...
    for epoch in range(1000):
        ...
        for i, (vs, y) in tqdm.tqdm(enumerate(train), total=len(train), desc=f"Epoch: {epoch} Train"):
            if i > len(train):
                break
        ...
        torch.save(model_state_dict, f"single_model_state_dict_{epoch=}.pth")
        torch.save(probe_state_dict, f"single_probe_state_dict_{epoch=}.pth")
...
class SolarDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        for i in time_index_ordering:
            ...
            examples = self.get_examples(pd.Timestamp(timestamp), num_examples=32)
            for example in examples:
                ...
                metadata = torch.from_numpy(np.asarray(metadata))
                metadata = torch.nan_to_num(metadata, 0.0)
                yield {"metoffice": metoffice, "metadata": metadata}, pv
        ...
        try:
            ...
            for ss_id in ids:
                ...
                if metoffice_system_data.time.shape[0] != 3:
                    continue
                if metoffice_system_data.projection_x_coordinate.shape[0] != 32 or metoffice_system_data.projection_y_coordinate.shape[0] != 32:
                    continue
                ...
                if np.sum(pv_system_generation) > 5: # Over 5 is more than 1 per one generally
                    continue
                pv_system_generation = np.asarray([pv_system_generation[2]]) # For the middle one
